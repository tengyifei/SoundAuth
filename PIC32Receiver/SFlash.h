...
extern unsigned int upper_128[128];		/* global array to store read data */
												/* to upper RAM area from 80H - FFH */

#define DF_CS			PORTBbits.RB1
#define DF_TCS			TRISBbits.TRISB1


// peripheral configurations
#define SPI_CONF    0x8120  	// SPI on, 8-bit master,CKE=1,CKP=0
#define SPI_BAUD_FAST    5		//15 //5 //10   //15      // clock divider Fpb/(2 * (15+1))
//#define SPI_BAUD_SLOW    71      // clock divider Fpb/(2 * (15+1))
//#define SPI_BAUD_SLOW    32      // clock divider Fpb/(2 * (15+1))
...
int DF_WriteSPI2( int i); 
void DF_WriteEnable( void);
void DF_Init( void);
int DF_ReadStatus( void);
int DF_ReadW(unsigned int address);
int DF_ReadD( int address);
void DF_SectorErase(int sector);
void DF_WriteW( int address, int data);
void DF_WritePage( int address, unsigned short int *ptr );

#define CANSPI_CS    PORTBbits.RB1       // select line for Serial EEPROM
#define CANSPI_TCS   TRISBbits.TRISB1    // tris control for DF_CS pin
...
#define CANSTAT 0x0e
#define CANCTRL 0x0f
#define SetNormalMode           0x00
#define SetSleepMode            0x20
#define SetLoopbackMode         0x40
#define SetListenMode           0x60
#define SetConfigurationMode    0x80
#define CLKEN   0x04
#define CNF1    0x2a
#define CNF2    0x29
#define CNF3    0x28
...
#define CAN_250kbps 1       //Based on 8 MHz Fosc
